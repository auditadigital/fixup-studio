...
```
Leé `auditoria-[id].html` (o los `scores` desde la base: `node scripts/db.mjs get "[id|업체명]"`, corriendo en `engine/`). Extraé:
...
## Paso 5 — Actualizar la base (Supabase vía `db.mjs`)

> Fuente de la verdad: tabla `prospectos` en Supabase. NO edites `prospectos.json`. Corré desde `engine/`.

1. **Patch** del prospecto:
   ```bash
   node scripts/db.mjs patch "[id|업체명]" '{"estado":"propuesta-enviada","fecha_propuesta":"[YYYY-MM-DD]","plan_recomendado":"[기본|성장|프리미엄]","precio_propuesto":[precio_del_plan]}'
   ```
2. **Adjuntá el reporte**:
   ```bash
   node scripts/db.mjs add-report "[id|업체명]" "제안서" "[output_path-pdf]"
   ```
...
> - Supabase → estado: propuesta-enviada
>
