---
name: fixup-pipeline
description: "Genera el dashboard HTML del pipeline de ventas de Fixup Studio leyendo prospectos.json. Muestra contadores por etapa, lista de prospectos con estados coreanos y colores, scores y monto cerrado en ₩. Triggers: 'abrí el pipeline', 'ver prospectos', 'dashboard de ventas', '파이프라인 보기'."
---

# Dashboard de Pipeline — 파이프라인 (Fixup Studio / 픽스업 스튜디오)

Adaptación de `dashboard-propuestas` de LATAM. Genera `dashboard.html` reflejando el estado real de `prospectos.json`. **Resultado SIEMPRE:** un HTML fresco. Idioma de cara al cliente: coreano (este dashboard es interno, pero usa labels coreanos).

> Dependencia: `fixup-reports/dashboard.html` + `assets/shared.css`.

---

## Paso 1 — Leer prospectos.json

Si no existe, crealo: `{ "prospectos": [] }`.
...
El operador cambia estados hablando: "Actualizá [업체명] a mini-enviada" / "Actualizá [업체명] a cerrado, monto 650000". Modificá `prospectos.json` y regenerá el dashboard si lo piden.
