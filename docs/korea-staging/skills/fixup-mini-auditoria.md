...
## Paso 6 — Actualizar prospectos.json

1. Leé `prospectos.json`.
2. Buscá el prospecto por `id`:
   - **Existe:** actualizá `scores_mini` (≈ score_mini), `fecha_mini`, `estado` → `"mini-lista"`.
   - **No existe:** agregá objeto nuevo con los campos disponibles (`nombre`, `id`, `rubro`, `zona`, `instagram`, `naver_place`, `kakao`); `null` lo desconocido.
3. Agregá a `reportes[]`: `{ "label": "미니 진단", "url": "[output_dir]/mini-auditoria-[id].png" }` (ruta local; sin hosting — ver `docs/data-plane.md`).
4. Guardá con Write.
...
> - prospectos.json → estado: mini-lista
>
