...
Leé `prospectos.json`, buscá `id`. Si existe `scores_mini`:
...
Actualizá `prospectos.json` con `scores` = scores del full audit.
...
## Paso 7 — Actualizar prospectos.json

Actualizá la entrada del `id`: `scores` (full), `fecha_completa`, `estado` → según corresponda (`propuesta-enviada` si seguís con la propuesta de plan mensual, o el estado que indique el operador). Agregá a `reportes[]`: `{ "label": "완전 진단", "url": "[output_dir]/auditoria-[id].pdf" }` (ruta local; sin hosting — ver `docs/data-plane.md`).
